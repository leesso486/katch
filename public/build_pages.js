...
<div class="admin-sidebar">
    ...
    <nav class="as-nav" style="padding: 20px 15px;">
        ...
        <a href="teacher_dashboard.html" class="as-nav-item \${activeId==='dash'?'active':''}" style="display:flex; align-items:center; gap:12px; padding:12px 15px; color:#cbd5e1; border-radius:10px; font-size:14px; font-weight:\${activeId==='dash'?'700':'600'}; text-decoration:none; margin-bottom:20px;"><i class="fas fa-home" style="width:20px; text-align:center;"></i> 대시보드 홈</a>

        <div style="font-size:11px; font-weight:800; color:#64748b; margin:10px 0 10px 10px; letter-spacing:1px;">학습 관리</div>
        <a href="teacher_grading.html" class="as-nav-item \${activeId==='grade'?'active':''}" style="display:flex; align-items:center; gap:12px; padding:10px 15px; color:#cbd5e1; border-radius:8px; font-size:14px; font-weight:\${activeId==='grade'?'700':'600'}; text-decoration:none; margin-bottom:5px;"><i class="fas fa-check-double" style="width:20px; text-align:center;"></i> 서술형 채점 / 피드백</a>
        <a href="admin_gvr_analytics.html" class="as-nav-item \${activeId==='gvr'?'active':''}" style="display:flex; align-items:center; gap:12px; padding:10px 15px; color:#cbd5e1; border-radius:8px; font-size:14px; font-weight:\${activeId==='gvr'?'700':'600'}; text-decoration:none; margin-bottom:5px;"><i class="fas fa-chart-line" style="width:20px; text-align:center;"></i> 주간 GVR 분석</a>
        <a href="teacher_dashboard.html" class="as-nav-item" style="display:flex; align-items:center; gap:12px; padding:10px 15px; color:#cbd5e1; border-radius:8px; font-size:14px; font-weight:600; text-decoration:none; margin-bottom:5px;"><i class="fas fa-users-cog" style="width:20px; text-align:center;"></i> 수강생 조회</a>
        
        <div style="font-size:11px; font-weight:800; color:#64748b; margin:25px 0 10px 10px; letter-spacing:1px;">콘텐츠 관리</div>
        <a href="admin_input.html" class="as-nav-item \${activeId==='input'?'active':''}" style="display:flex; align-items:center; gap:12px; padding:10px 15px; color:#cbd5e1; border-radius:8px; font-size:14px; font-weight:\${activeId==='input'?'700':'600'}; text-decoration:none; margin-bottom:5px;"><i class="fas fa-cloud-upload-alt" style="width:20px; text-align:center;"></i> 문항 업로드 (DB)</a>
        <a href="problem_bank.html" class="as-nav-item \${activeId==='bank'?'active':''}" style="display:flex; align-items:center; gap:12px; padding:10px 15px; color:#cbd5e1; border-radius:8px; font-size:14px; font-weight:\${activeId==='bank'?'700':'600'}; text-decoration:none; margin-bottom:5px;"><i class="fas fa-layer-group" style="width:20px; text-align:center;"></i> 문제은행 출제</a>
        
        <div style="font-size:11px; font-weight:800; color:#64748b; margin:25px 0 10px 10px; letter-spacing:1px;">커뮤니티</div>
        <a href="teacher_community.html" class="as-nav-item \${activeId==='comm'?'active':''}" style="display:flex; align-items:center; gap:12px; padding:10px 15px; color:#cbd5e1; border-radius:8px; font-size:14px; font-weight:\${activeId==='comm'?'700':'600'}; text-decoration:none;"><i class="fas fa-store" style="width:20px; text-align:center;"></i> 지식 마켓 & 라운지</a>
    </nav>
</div>
`;
}
...
// ============================
// 1. teacher_grading.html
// ============================
const content_grading = `
\${GLOBAL_HEAD}
\${GLOBAL_TOPBAR}
            ...
            <div style="display:grid; grid-template-columns:1fr 400px; gap:20px;">
                <div class="tw-card">
                    ...
                    <div style="border:2px dashed #cbd5e1; border-radius:12px; padding:40px; text-align:center; background:#f8fafc; margin-bottom:20px; cursor:pointer;" onmouseover="this.style.borderColor='#3b82f6'; this.style.background='#eff6ff'" onmouseout="this.style.borderColor='#cbd5e1'; this.style.background='#f8fafc'">
                        ...
                        <div style="font-size:16px; font-weight:800; color:#1e293b;">한글(HWP) 복붙 또는 PDF 드래그 & 드롭</div>
                        <div style="font-size:13px; color:#64748b; margin-top:5px;">AI가 지문, 문항, 보기를 자동으로 분류하여 입력창에 채워줍니다.</div>
                    </div>
                ...
                <div class="tw-card" style="background:#f8fafc;">
                    ...
                    <div style="font-size:13px; line-height:1.6; color:#475569; margin-bottom:20px;">
                        업로드된 지문과 문항을 분석하여 KATCH 데이터베이스 구조에 맞게 카테고리를 자동 분류합니다. (강사 수정 가능)
                    </div>
                    ...
                    <button class="tw-btn solid-blue" style="width:100%; font-size:16px; padding:15px;"><i class="fas fa-database"></i> 문제은행 DB에 저장하기 (10 P 획득)</button>
...
<div style="display:flex; width:100%;">
    ...
    <div class="main-wrapper">
        <div class="dash-container">
            ...
            <div style="display:grid; grid-template-columns:300px 1fr 340px; gap:20px;">
                ...
                <div class="tw-card" style="padding:0; overflow:hidden;">
                    ...
                    <div style="padding:20px; display:flex; flex-direction:column; gap:15px;">
                        <!-- Item -->
                        <div style="border:1px solid #cbd5e1; border-radius:10px; padding:15px; position:relative;">
                            <div style="position:absolute; top:15px; right:15px;"><button class="tw-btn outline" style="padding:4px 10px; font-size:11px;">+ 바구니에 담기</button></div>
                            <div style="display:flex; gap:10px; margin-bottom:10px;"><span style="background:#e0e7ff; color:#4f46e5; padding:3px 8px; border-radius:4px; font-size:11px; font-weight:800;">빈칸추론</span><span style="background:#f1f5f9; color:#475569; padding:3px 8px; border-radius:4px; font-size:11px; font-weight:800;">난이도: 상</span><span style="background:#f1f5f9; color:#475569; padding:3px 8px; border-radius:4px; font-size:11px; font-weight:800;">2026 수특 p.14</span></div>
                ...
                <!-- 출제 바구니 및 클리닉 생성 -->
                <div class="tw-card" style="background:#eff6ff; border-color:#bfdbfe;">
                    <h3 style="font-size:16px; font-weight:800; margin:0 0 15px 0;"><i class="fas fa-shopping-basket text-blue-500"></i> 미니테스트 (바구니)</h3>
                    <div style="background:white; border-radius:8px; padding:15px; border:1px solid #bfdbfe; margin-bottom:15px;">
                        담은 문항: <strong>15문제</strong> (빈칸 10, 서술형 5)
                    </div>
                    ...
                    <button class="tw-btn solid-blue" style="width:100%; font-size:15px;"><i class="fas fa-paper-plane"></i> 클래스 수강생에게 즉시 배포</button>
                    <button class="tw-btn outline" style="width:100%; font-size:15px; margin-top:10px;"><i class="fas fa-file-pdf"></i> PDF 및 정답지 인쇄</button>
                </div>
            </div>
        </div>
    </div>
...
fs.writeFileSync('teacher_grading.html', content_grading, 'utf8');
fs.writeFileSync('admin_gvr_analytics.html', content_gvr, 'utf8');
fs.writeFileSync('admin_input.html', content_input, 'utf8');
fs.writeFileSync('problem_bank.html', content_bank, 'utf8');

// Update teacher_community.html layout while preserving its internal style and structure.
// I'll re-wrap community inside the unified layout.
const community_original = fs.readFileSync('teacher_community.html', 'utf8');
// Naively extract the <div class="c-container"> and forward...
const c_match = community_original.indexOf('<div class="c-container">');
const community_content = community_original.substring(c_match).split('</body>')[0];

const content_community = \`
\${GLOBAL_HEAD}
<style>
...
.l-side-box { background: white; border-radius: 16px; padding: 20px; border: 1px solid #e2e8f0; margin-bottom: 20px; }
</style>
\${GLOBAL_TOPBAR}
<div style="display:flex; width:100%;">
    \${getSidebar('comm')}
...
fs.writeFileSync('teacher_community.html', content_community, 'utf8');
